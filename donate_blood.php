...
$bloodqty = $weight = $contactNumber = $address = $disease = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($bloodqty) || empty($weight) || empty($contactNumber) || empty($address) || empty($disease)) {
        $errors[] = "All fields are required.";
    } else {
        // Prepare the insert statement
        $sql = "INSERT INTO donors (bloodqty, weight, contact_no, address, disease, user_id) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssi", $bloodqty, $weight, $contactNumber, $address, $disease, $userID);

        // Execute the statement
        if ($stmt->execute()) {
            // Redirect to donation history page after successful submission
...
<body>
    ...
    <section class="sectionBody">
        ...
        <form id="donateForm" method="POST">
            ...
            <input type="text" id="bloodgroup" name="bloodgroup" value="<?php echo $bloodgroup; ?>" readonly><br>
            ...
            <input type="text" id="disease" name="disease" placeholder="Please mention the name of disease, if any"
                value="<?php echo $disease; ?>"><br>
