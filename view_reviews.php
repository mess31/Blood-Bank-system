...
<body>
    ...
    <section class="reviewHistory">
    <h2 align="center">Reviews and Feedbacks</h2>
    <br>
    <?php
    // Fetch all reviews with related user and campaign information
    $sql = "SELECT 
            r.review_id, 
            r.star_count, 
            r.message, 
            r.review_date, 
            c.camp_img,
            r.item_id AS campaign_id
        FROM 
            reviews r
        JOIN 
            campaign c ON r.item_id = c.campaign_id
        WHERE 
            r.user_id = $id
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr>
                    <td>" . $serialNumber . "</td>
                    <td>
                        <span class='rectangulardiv'>
                            <img src='./images/campaignpic/" . $row["camp_img"] . "' alt='error'>
                        </span>
                    </td>";
            ...
            echo "<td class='message'>" . $row["message"] . "</td>
                    ...
                    <td>
                        <a href='feedback_info.php?campaign_id=" . $row["campaign_id"] . "' style='text-decoration: none; font-size: 20px; cursor: pointer;'><i class='fa-solid fa-circle-info'></i></a>&nbsp
                        <a href='delete_feedback.php?review_id=" . $row["review_id"] . "' style='text-decoration: none; font-size: 20px; cursor: pointer;'>
